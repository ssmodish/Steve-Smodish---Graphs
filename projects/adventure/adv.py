...
def explore_map():
    ...
    def explore_DFT():
        ...
        while '?' in world_map[player.currentRoom.id].values():
            ...
            if 'w' in world_map[player.currentRoom.id]:
                if world_map[player.currentRoom.id]['w'] == '?':
                    player.travel('w')
                    traversalPath.append('w')
                    add_room_to_map()
                    world_map[prev_room_id]['w'], world_map[player.currentRoom.id]['e'] = player.currentRoom.id, prev_room_id
            if 'n' in world_map[player.currentRoom.id]:
                if world_map[player.currentRoom.id]['n'] == '?':
                    player.travel('n')
                    traversalPath.append('n')
                    add_room_to_map()
                    world_map[prev_room_id]['n'], world_map[player.currentRoom.id]['s'] = player.currentRoom.id, prev_room_id
            if 'e' in world_map[player.currentRoom.id]:
                if world_map[player.currentRoom.id]['e'] == '?':
                    player.travel('e')
                    traversalPath.append('e')
                    add_room_to_map()
                    world_map[prev_room_id]['e'], world_map[player.currentRoom.id]['w'] = player.currentRoom.id, prev_room_id
            if 's' in world_map[player.currentRoom.id]:
                if world_map[player.currentRoom.id]['s'] == '?':
                    player.travel('s')
                    traversalPath.append('s')
                    add_room_to_map()
                    world_map[prev_room_id]['s'], world_map[player.currentRoom.id]['n'] = player.currentRoom.id, prev_room_id

    explore_DFT()

    # if there are no undefined exits from the current room
    # run a bfs on the graph to find the nearest undefined exit and move player there
